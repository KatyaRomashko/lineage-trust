...
def data_source_to_dataset(
    ...
    Args:
        ...
        repo_config: Optional RepoConfig for resolving physical data source URIs
    ...
    dataset_namespace, dataset_name = _resolve_data_source_identity(
...
def _resolve_data_source_identity(
    data_source: "DataSource",
    ...
    """
    Resolve the OpenLineage namespace and dataset name for a data source.

    For database-backed sources (e.g. PostgreSQLSource), uses the offline
    store connection details from repo_config to produce a URI-based identity
    (e.g. postgres://host:port, database.table) that matches identities
    emitted by other tools like Spark and MLflow.

    Falls back to the Feast namespace and source name when repo_config is
    unavailable or the source type is not a database source.

    Args:
        data_source: Feast DataSource
        default_namespace: Fallback namespace
        default_name: Fallback dataset name (typically data_source.name)
        repo_config: Optional RepoConfig with offline store connection info

    Returns:
        Tuple of (namespace, dataset_name)
    """
    if repo_config is not None:
        try:
            ...
            if isinstance(data_source, PostgreSQLSource):
                offline = repo_config.offline_store
                host = getattr(offline, "host", None)
                port = getattr(offline, "port", 5432)
                database = getattr(offline, "database", None)

                if host:
                    ns = f"postgres://{host}:{port}"
                    table = data_source._postgres_options._table
                    if not table and data_source._postgres_options._query:
                        import re

                        match = re.search(
                            r"\bFROM\s+(\w+)",
                            data_source._postgres_options._query,
                            re.IGNORECASE,
                        )
                        if match:
                            table = match.group(1)
